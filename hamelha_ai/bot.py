...
def format_menu():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📱 9:16 Shorts",callback_data="fmt:shorts"),InlineKeyboardButton(text="⬛ 1:1 مربع",callback_data="fmt:square")],[InlineKeyboardButton(text="🖥️ أفقي 16:9",callback_data="fmt:landscape")]])
...
async def consume(m):
    ...
    if not await db.spend(m.from_user.id): await m.answer("⛔ انتهى رصيدك.",reply_markup=menu()); return False
    return True
@router.message(CommandStart())
async def start(m:Message):
    u=await user(m); match=re.search(r"start(?:=|\s+)ref_([A-Za-z0-9]+)",m.text or "")
    ...
    await m.answer(f"🚀 <b>Hamelha AI Ultra</b>\n\nاستوديو فيديو متكامل داخل Telegram.\n\n✍️ نص → فيديو + صوت عربي\n🖼️ صور → فيديو\n🎬 تحرير ومقاسات\n✨ تحسين الفيديو\n🎵 فيديو → MP3\n⬇️ تحميل رابط\n👥 إحالات\n\n🎁 رصيدك: <b>{u[2]}</b>",reply_markup=menu())
@router.callback_query(F.data=="help")
async def help_cb(c:CallbackQuery): await c.answer(); await c.message.answer("📚 <b>الخدمات</b>\n\n✍️ النص: اختر الخدمة ثم أرسل النص.\n🖼️ الصور: أرسل الصور ثم /done.\n🎬 التحرير: اختر المقاس ثم أرسل الفيديو.\n✨ التحسين: أرسل الفيديو مباشرة.\n🎵 الصوت: أرسل الفيديو لاستخراج MP3.\n⬇️ التحميل: أرسل رابطًا عامًا تملك حق تنزيله.",reply_markup=menu())
@router.callback_query(F.data=="balance")
async def balance(c:CallbackQuery): u=await db.user(c.from_user.id,c.from_user.username); await c.answer(); await c.message.answer(f"💎 <b>رصيدك:</b> {u[2]}\n👥 الإحالات: {u[3]}\n🔑 كودك: <code>{u[4]}</code>",reply_markup=menu())
@router.callback_query(F.data=="ref")
async def ref(c:CallbackQuery): u=await db.user(c.from_user.id,c.from_user.username); me=await c.bot.get_me(); await c.answer(); await c.message.answer(f"👥 <b>دعوة الأصدقاء</b>\n\n<code>https://t.me/{me.username}?start=ref_{u[4]}</code>\n\n🎁 كل إحالة جديدة تمنح صاحب الدعوة +2 عمليات.")
...
async def text_handler(m:Message,state:FSMContext):
    ...
    status=await m.answer("⏳ جاري إنشاء الفيديو... 0%")
    jid=await db.job(m.from_user.id,"text_to_video",t)
    try:
        out=await text_to_video(t,m.from_user.id,voice=True); await db.set_job(jid,"done",str(out)); await status.edit_text("✅ اكتمل إنشاء الفيديو، جارٍ إرساله..."); await m.answer_video(FSInputFile(out),caption="🎬 تم إنشاء الفيديو")
    except Exception as e: await db.set_job(jid,"failed",error=str(e)); await status.edit_text("❌ فشل إنشاء الفيديو. راجع سجل Termux لمعرفة السبب."); log.exception("text_to_video failed: %s",e)
@router.callback_query(F.data=="images")
async def images_mode(c:CallbackQuery,state:FSMContext): await c.answer(); await state.set_state(ImgState.collecting); await state.update_data(paths=[]); await c.message.answer("🖼️ أرسل الصور واحدة تلو الأخرى، ثم /done. الحد 40 صورة.")
...
async def finish_images(m:Message,state:FSMContext):
    ...
    if not await consume(m):return
    status=await m.answer("⏳ جاري صنع الفيديو من الصور...");jid=await db.job(m.from_user.id,"images_to_video",str(paths))
    try:
        out=await images_to_video(paths,m.from_user.id);await db.set_job(jid,"done",str(out));await status.edit_text("✅ اكتمل الفيديو، جارٍ إرساله...");await m.answer_video(FSInputFile(out),caption="🎬 تم إنشاء فيديو الصور")
    except Exception as e:await db.set_job(jid,"failed",error=str(e));await status.edit_text("❌ فشل إنشاء فيديو الصور. راجع سجل Termux.");log.exception("images failed: %s",e)
...
async def enhance(c:CallbackQuery,state:FSMContext):await c.answer();await state.set_state(ModeState.mode);await state.update_data(mode="enhance");await c.message.answer("✨ أرسل الفيديو الآن.")
...
async def video_handler(m:Message,state:FSMContext,bot:Bot):
    ...
    if not await consume(m):return
    p=Path(settings.work_dir)/str(m.from_user.id)/f"input_{m.video.file_unique_id}.mp4";p.parent.mkdir(parents=True,exist_ok=True);await bot.download(m.video,destination=p);status=await m.answer("⏳ جاري المعالجة...");jid=await db.job(m.from_user.id,"extract_audio" if mode=="audio" else "edit_video",str(p))
    try:
        out=await extract_audio(p,m.from_user.id) if mode=="audio" else await edit_video(p,m.from_user.id,"enhance" if mode=="enhance" else fmt);await db.set_job(jid,"done",str(out));await status.edit_text("✅ اكتملت المعالجة، جارٍ إرسال النتيجة...")
    ...
    except Exception as e:await db.set_job(jid,"failed",error=str(e));await status.edit_text("❌ فشلت المعالجة. راجع سجل Termux.");log.exception("video failed: %s",e)
...
async def do_download(m,url):
    if not await consume(m):return
    status=await m.answer("⬇️ جاري التحميل...");jid=await db.job(m.from_user.id,"download",url)
    try:out=await download(url,m.from_user.id);await db.set_job(jid,"done",str(out));await status.edit_text("✅ اكتمل التحميل، جارٍ الإرسال...");await m.answer_video(FSInputFile(out),caption="⬇️ تم التحميل")
    except Exception as e:await db.set_job(jid,"failed",error=str(e));await status.edit_text("❌ تعذر التحميل. الرابط قد يكون غير عام أو الموقع منع الطلب.");log.exception("download failed: %s",e)
...
async def main():
    ...
    os.makedirs(settings.work_dir,exist_ok=True);await db.init();bot=Bot(settings.bot_token,default=DefaultBotProperties(parse_mode=ParseMode.HTML));dp=Dispatcher();dp.include_router(router);log.info("Hamelha AI Ultra started");await dp.start_polling(bot)
